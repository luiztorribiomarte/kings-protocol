...
(function () {
  ...
  function computeMoodHabitInsights(keys, energyValues, habitStats) {
    ...
    else if (avgHi != null && hi.length === 1) line1 = `You had 1 high-energy day (7–10): ${avgHi}% habit completion.`;

    if (avgLo != null && lo.length >= 2) line2 = `On low-energy days (1–4), you averaged ${avgLo}% completion.`;
    else if (avgLo != null && lo.length === 1) line2 = `You had 1 low-energy day (1–4): ${avgLo}% completion.`;

    let compare = "";
    if (avgHi != null && avgLo != null) {
      const diff = avgHi - avgLo;
      const abs = Math.abs(diff);
      if (abs >= 5) compare = diff > 0 ? `That's ${abs} points higher when your energy is high.` : `That's ${abs} points higher when your energy is low.`;
              ...
              .map((d) => {
                ...
                return `
                  <div style="text-align:center; padding:10px 8px; border-radius:14px; border:1px solid rgba(255,255,255,0.12); background:${
                    ...
                    <div style="font-size:1.25rem; margin-top:6px;">${entry?.mood ? entry.mood : "—"}</div>
                    <div style="margin-top:6px; font-size:0.85rem; color:#e5e7eb;">${entry?.energy ? `${entry.energy}` : "—"}</div>
                  </div>
                `;
              })
              .join("")}
