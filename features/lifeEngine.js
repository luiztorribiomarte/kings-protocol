// ============================================
// LIFE ENGINE 7.5.2 — LOCAL DATE HARD SYNC
// FIXES:
// - Restores original UI rendering (dailyStatus, trendChart)
// - Normalizes ALL date keys to local YYYY-MM-DD
// - Habits % now matches habitCompletions keys
// - Trend + DNA + Life Score update correctly
// ============================================
...
(function () {
  ...
  function localKeyFromDate(d = new Date()) {
  ...
  function normalizeKey(input) {
    ...
    // Date object
    if (input instanceof Date) return localKeyFromDate(input);

    // other strings like "2/14/2026" etc
    try {
      const d = new Date(input);
      if (!isNaN(d.getTime())) return localKeyFromDate(d);
    } catch {}

    // fallback: today
    return localKeyFromDate(new Date());
  }
  ...
  function getAllDaysFromStorage() {
    ...
    try { habitData = JSON.parse(localStorage.getItem("habitCompletions") || "{}"); } catch {}
    try { moodData = JSON.parse(localStorage.getItem("moodData") || "{}"); } catch {}
    try { todoHistory = JSON.parse(localStorage.getItem("todoHistory") || "{}"); } catch {}

    const keys = new Set();

    Object.keys(habitData || {}).forEach(k => keys.add(normalizeKey(k)));
    Object.keys(moodData || {}).forEach(k => keys.add(normalizeKey(k)));
    Object.keys(todoHistory || {}).forEach(k => keys.add(normalizeKey(k)));
  ...
  function getHabitPercent(dateKey = todayKey()) {
    const k = normalizeKey(dateKey);
    try {
      if (typeof window.getDayCompletion === "function") {
        const result = window.getDayCompletion(k);
  ...
  function getEnergyPercent(dateKey = todayKey()) {
    ...
    try {
      ...
      const energy = moodData?.[k]?.energy ?? 0;
  ...
  function getTaskPercentForDay(dateKey = todayKey()) {
    ...
    try {
      ...
      if (hist?.[k]?.percent != null) return hist[k].percent;
    } catch {}
  ...
  // ---------- Reactive Sync ----------
  function refreshAll() {
    // If habits/mood modules load slightly after, this prevents a blank dashboard.
    renderLifeScore();
  ...
  console.log("Life Engine 7.5.2 loaded (local-date hard sync)");
})();
